...
export function Navbar() {
  ...
  return <header className={`site-nav ${scrolled ? 'is-scrolled' : ''}`}>
    ...
    {open && <nav className="mobile-menu shell flex flex-col gap-5 border-t border-border py-6 md:hidden" aria-label="Mobile primary">{navItems.map(([label, href]) => <Link key={href} href={href} aria-current={isActive(href) ? 'page' : undefined} className={isActive(href) ? 'nav-link-active' : ''} onClick={() => setOpen(false)}>{label}</Link>)}<Link href="/contact" onClick={() => setOpen(false)} className="button-primary inline-flex min-h-10 items-center justify-center rounded-sm px-4 py-2 text-sm">Let&apos;s Talk <ArrowUpRight className="ml-2 size-4 shrink-0" /></Link></nav>}
  </header>
}

export function Footer() { return <footer className="footer"><div className="shell"><div className="grid gap-10 border-b border-border pb-10 md:grid-cols-[1.5fr_repeat(3,1fr)]"><div><Logo /><p className="mt-4 max-w-xs text-sm text-muted-foreground">Move fast. Ship better. Senior engineering for ambitious teams.</p></div><div className="footer-col"><p className="footer-label">Company</p>{[['About','/about'],['Approach','/approach'],['Work','/work'],['Contact','/contact']].map(([l,h])=><Link key={h} href={h}>{l}</Link>)}</div><div className="footer-col"><p className="footer-label">Solutions</p>{[['AI & Automation','/solutions/ai-automation'],['Software Engineering','/solutions/software-engineering'],['Mobile','/solutions/mobile'],['Cloud & DevOps','/solutions/cloud-devops']].map(([l,h])=><Link key={h} href={h}>{l}</Link>)}</div><div className="footer-col"><p className="footer-label">Connect</p><a href="#" aria-label="LinkedIn placeholder">LinkedIn</a><a href="#" aria-label="GitHub placeholder">GitHub</a><a href="mailto:hello@vectorship.ai">Email</a></div></div><div className="flex flex-col gap-3 pt-6 text-xs text-muted-foreground md:flex-row md:items-center md:justify-between"><span>© 2026 VectorShip AI Solutions. All rights reserved.</span><span className="flex gap-5"><Link href="/privacy">Privacy Policy</Link><Link href="/terms">Terms of Service</Link></span></div></div></footer> }
