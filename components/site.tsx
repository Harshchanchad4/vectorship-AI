...
export function PageHero({ eyebrow='NEXIO AI SOLUTIONS', title, copy, children }: { eyebrow?: string; title: string; copy: string; children?: React.ReactNode }) { return <section className="page-hero shell"><p className="eyebrow"><span className="pulse-dot" />{eyebrow}</p><h1 className="text-balance">{title}</h1><p className="page-lede">{copy}</p>{children}</section> }
...
const REVEAL_SELECTOR = '.capability-card, .profile-card, .case-card, .booking-card, .why-item, .trust-item, .process-step, .tech-group, .overlap-banner, .role-card'
